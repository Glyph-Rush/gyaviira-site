...
interface PulseLyricsProps {
    isRhythmOpen: boolean;
}

const PulseLyrics: React.FC<PulseLyricsProps> = ({ isRhythmOpen }) => {
    ...
    const getBottomPosition = () => {
        if (isChatPage) {
            return isRhythmOpen ? 'bottom-80' : 'bottom-52 md:bottom-48';
        }
        return isRhythmOpen ? 'bottom-64' : 'bottom-44';
    };

    return (
        <div className={`fixed ${getBottomPosition()} left-6 z-50 transition-all duration-500`}>
            <AnimatePresence>
                {!isOpen && (
                    <motion.button
